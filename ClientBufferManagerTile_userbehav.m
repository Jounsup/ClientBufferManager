...
inip = [zeros(1,floor(tileCount/4)) ones(1,floor(tileCount/2)) zeros(1,floor(tileCount/4))];
p = inip; 
% grd = 1;
for bb=1:bufGof
...
%             p = conv(p,ones(1,3),'same'); p = p/sum(p);
    ...
    figure(1); bar3(stateMatrix); axis([0 bufGof 0 tileCount 0 length(r)]); drawnow;
    F(ss) = getframe;
    R(ss+1) = max([1 BW]);
